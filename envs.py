...
def make_env(env_id, seed, rank, log_dir, add_timestep, allow_reset=False, pixels=False):
    def _thunk():
        ...
        if pixels:
            env = pixel_wrapper.PixelObservationWrapper(env, stack=1, img_width=64)
        ...
        if log_dir is not None:
            monitor_path = os.path.join(log_dir, "{}.monitor.csv".format(str(rank)))
            print("making monitor logging to {}".format(monitor_path))
            env = bench.Monitor(env, monitor_path,
